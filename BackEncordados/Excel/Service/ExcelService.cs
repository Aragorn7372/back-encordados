...
) : IExcelService
{
    ...
    public async Task<byte[]> ExportTournamentAsync(Ulid userId, Ulid tournamentId)
    {
        ...
        var pedidos = await excelRepository.GetPedidosByTournamentAsync(tournamentId);

        var playerIds = pedidos.Select(p => p.PlayerId).Distinct().ToList();
        var users = await excelRepository.GetUsersByIdsAsync(playerIds);

        var data = pedidos
            .GroupBy(p => p.PlayerId)
            .Select(g => g.ToTournamentExcelRowDto(users))
            .OrderBy(r => r.Username)
            .ToList();

        var tournament = await excelRepository.GetTournamentByIdAsync(tournamentId);
        var tournamentName = tournament?.Title ?? $"Torneo {tournamentId}";
    ...
    public async Task<byte[]> ExportAdvancedAsync(Ulid userId, Ulid tournamentId, List<string> types, string role)
    {
        ...
        {
            var users = await excelRepository.GetUsersByTournamentAsync(tournamentId);
            data.Users = users.Select(u => u.ToExcelUsersDto()).ToList();
        }

        if (validTypes.Contains("materials"))
        {
            var materials = await excelRepository.GetMaterialsByTournamentAsync(tournamentId);
            data.Materials = materials.Select(m => m.ToExcelMaterialsDto()).ToList();
        }

        if (validTypes.Contains("cuerdas"))
        {
            var cuerdas = await excelRepository.GetCuerdasByTournamentAsync(tournamentId);
            data.Cuerdas = cuerdas.Select(c => c.ToExcelCuerdasDto()).ToList();
        }
        ...
        if (validTypes.Contains("tournament"))
        {
            ...
            if (tournament != null)
            {
                data.Tournament.Add(tournament.ToExcelTournamentDto());
            }
        }
        ...
        {
            var pedidos = await excelRepository.GetPedidosByTournamentAsync(tournamentId);
            data.Pedidos = pedidos.Select(p => p.ToExcelPedidosDto()).ToList();

            var pedidoIds = pedidos.Select(p => p.Id).ToList();
            var lineas = await excelRepository.GetPedidoLineasByPedidoIdsAsync(pedidoIds);
            data.PedidoLineas = lineas.Select(l => l.ToExcelPedidoLineasDto()).ToList();
        }

        var tournamentEntity = await excelRepository.GetTournamentByIdAsync(tournamentId);
        var tournamentName = tournamentEntity?.Title ?? $"Torneo {tournamentId}";
